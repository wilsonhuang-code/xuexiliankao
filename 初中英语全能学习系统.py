...
# 确保桌面目录在sys.path中（修复双击启动时import失败的问题）
import os, sys
...
class EnglishLearningTool(QMainWindow):
        ...
        for w, m in self.word_lib.items():
            if grade != '全部':
                ...
                if grade not in item_grade:
                    continue
            ph = phonetic_dict.get(w, ""); self.word_list.addItem(f"{w} [{ph}] - {m}" if ph else f"{w} - {m}")
    ...
    def _setup_essay_tab(self):
        """设置英语作文 Tab - 集成好句功能"""
        from PyQt5.QtWidgets import QTextEdit, QComboBox, QLabel, QPushButton, QGroupBox, QHBoxLayout, QVBoxLayout, QSplitter
        from composition_sentences_widget import CompositionSentencesWidget
        ...
        self.essay_guide.setTextInteractionFlags(Qt.NoTextInteraction)
        ...
        write_layout.addWidget(guide_group)
        ...
        # === 下方：好句参考区域 ===
        sentences_group = QGroupBox("📝 英语作文美句参考")
        sentences_layout = QVBoxLayout(sentences_group)
        
        # 创建好句组件（传递主窗口引用，用于插入句子）
        self.sentences_widget = CompositionSentencesWidget(self, self.current_grade if hasattr(self, 'current_grade') else '七年级')
        sentences_layout.addWidget(self.sentences_widget)
        
        splitter.addWidget(sentences_group)
        
        # 设置分割器比例（上方写作区域占2/3，下方好句区域占1/3）
        ...
        # 连接好句组件的插入信号（如果需要）
        if hasattr(self.sentences_widget, 'sentence_selected'):
            self.sentences_widget.sentence_selected.connect(self.insert_sentence_to_composition)
        
    def insert_sentence_to_composition(self, sentence):
        """将选中的好句插入到作文编辑器中"""
        if hasattr(self, 'essay_editor'):
            # 在光标位置插入句子
            self.essay_editor.insertPlainText(sentence + " ")
            # 显示提示
            self.statusBar().showMessage(f"已插入句子：{sentence[:50]}...", 3000)
